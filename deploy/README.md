...
1. Crie uma VM Compute Ubuntu 24.04 com IP publico e chave SSH.
...
4. Instale Docker, Docker Compose, Nginx e Certbot na VM. Copie este projeto para `/opt/bilca-event/app`.
...
8. Copie `nginx-bilca-event.conf` para `/etc/nginx/sites-available/bilca-event`, troque todas as ocorrencias de `SEU_DOMINIO`, habilite o site e confira com `sudo nginx -t`.
